...
const sizeSection int = 15000 * 10000
...
func New() Queue {
	q := Queue{
		new(sync.Mutex), make([]Message, sizeSection), 0, 0, sizeSection,
	}
	return q
}

// 这里应该优化成*Queue，否则copy
func (q Queue) PushTailCASBad(n Message) {
	for {
		if atomic.CompareAndSwapInt32(&q.hasp, 0, 1) {
			break
		}
		runtime.Gosched() // 让出CPU，避免自旋锁死
	}
	q.db[q.tail] = n
	q.tail++
	if q.tail > q.size {
		q.db = append(q.db, make([]Message, sizeSection)...)
		q.size += sizeSection
	}
	q.hasp = 0
	//atomic.StoreInt32(&q.hasp, 0)
}
func (q Queue) PushTailMutexBad(n Message) {
	q.mu.Lock()
	q.db[q.tail] = n
	q.tail++
	if q.tail > q.size {
		// 这里应该优化成*Queue，否则对copy的q操作不会反映到外面
		q.db = append(q.db, make([]Message, sizeSection)...)
		q.size += sizeSection
	}
	q.mu.Unlock()
...
func (q *Queue) PushTailCASFixed(n Message) {
	...
	q.hasp = 0
	// atomic.StoreInt32(&q.hasp, 0) // more safe
}
