// Example of using bpf to capture http:80 packets in OSX
// gcc -o a a.c && sudo  ./a en0
...
int main(int argc, char *argv[]) {
    ...
    if (set_filter(fd) < 0) err(EXIT_FAILURE, "set_filter");
...
int check_dlt(int fd) {
    ...
    switch (dlt) {
        case DLT_EN10MB:
            (void)fprintf(stderr, "Supported datalink type:%u\n", dlt);
            return 0;
        default:
            (void)fprintf(stderr, "Unsupported datalink type:%u (https://linux.die.net/man/7/pcap-linktype)\n", dlt);
            errno = EINVAL;
            return -1;
    }
}
...
void read_packets(int fd) {
    ...
    for ( ; ; ) {
        ...
        (void)printf("some packets received.\n");
        p = buf;
        while (p < buf + n) {
            bh = (struct bpf_hdr *)p;
