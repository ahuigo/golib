...
func ConvertObjectsByte2String(o interface{}) interface{} {
	refV := reflect.ValueOf(o)
	// var out interface{}
	ps := PtrSeen{}
	out := objectByte2String(refV, ps)
	return out
}

func objectByte2String(rv reflect.Value, ptrSeen PtrSeen) (out interface{}) {
	...
	case reflect.Array:
		...
		if t.Elem().Kind() == reflect.Uint8 {
			...
			n := rv.Len()
			for i := 0; i < n; i++ {
				s[i] = objectByte2String(rv.Index(i), ptrSeen)
			}
			return s
		}
		// return rv.Interface()
	...
	case reflect.Map:
		...
		for i := 0; mi.Next(); i++ {
			k := mi.Key()
			v := mi.Value()
			m[k.String()] = objectByte2String(v, ptrSeen)
		}
	...
	case reflect.Interface:
		...
		return objectByte2String(rv.Elem(), ptrSeen)
	case reflect.Struct:
		m := map[string]interface{}{}
		for i := 0; i < t.NumField(); i++ {
			...
			fv := rv.Field(i)
			// m[key] = fv.Interface()
			v := objectByte2String(fv, ptrSeen)
